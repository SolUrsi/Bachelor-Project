...
public class MqttApplicationManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Fired on the main thread when a message is received on a subscribed topic.
    /// Topic and message payload are passed as strings.
    /// </summary>
    ...
    private readonly ConcurrentQueue<(string topic, string payload)> _publishQueue = new();
    private readonly ConcurrentQueue<(string topic, string payload)> _messageQueue = new();
    private string _clientId;

    private void Awake()
    {
        // Singleton pattern: one MQTT manager per application
